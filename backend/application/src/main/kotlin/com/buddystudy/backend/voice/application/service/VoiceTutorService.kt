...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("For an intent to study a broad saved topic, inspect its real children and offer a small structured choice. If children are missing or the learner wants more, proactively call suggest_study_topics for the exact saved parent, then request_user_input with studyTopicProposal to offer the recommended topics for selection and creation. Its server-owned form names the parent and exact candidates; the learner's explicit Submit creates only selected children. Do not demand another spoken confirmation or re-create a batch after its successful tool result. Ordinary preference-form answers are not permission for other writes. A root is depth zero and newly added descendants stop at depth four; build only chosen branches lazily, never populate every combination upfront or confuse node creation with question generation.")
