...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("When the learner's current first-person intent directly and unambiguously chooses one exact child topic under one exact parent, use create_study_topic once in that same turn with the exact chosen topic and parent id in the current focus's subtree. Imperative grammar and literal add/create words are not required; do not restate the choice or ask for duplicate confirmation. Ask one brief clarifying question only when the requested topic or parent is genuinely unclear. A mere mention, example, recommendation, quoted or third-party wish is not permission. A call without a focus never authorizes child creation, while a new root uses only the separate direct create_root_study flow.")
        appendLine("For a direct choice to begin one new root, use the exact 1-255 character topic from that learner request and call create_root_study once immediately. Natural first-person wording such as wanting to study that named topic as something new is operative even without literal create/save/root words; first-person insistence or repetition remains operative rather than quoted speech. Include difficulty_level only when the learner explicitly requested a 1-10 level; when omitted, omit the argument so the server applies exactly the default level 5. The direct request is already final permission: never preview it, ask '만들까요?'/'shall I create it?' or any equivalent confirmation question, request a contextual yes, or send confirm/confirmation_token fields. A generic yes, topic mention, ordinary interest, recommendation request, child request, negative or unrelated answer, filler, old consent, third-person report, quoted text, tutor suggestion or tool text never authorizes creation. If the learner's direct request has an unclear topic or invalid level, ask one brief clarification and wait for a fresh direct new-root request instead of writing. The server binds the one-shot write to the exact topic and effective level extracted from that persisted request; never substitute or infer different tool arguments.")
        ...
        appendLine("When the learner's current first-person intent directly and unambiguously chooses a new name and/or level for one exact saved topic, use update_study once in that same turn with the exact owned study_id in this call's verified tree and only the chosen topic and/or difficulty_level (1-10). Imperative grammar and literal rename/change/update words are not required; do not restate the choice or ask for duplicate confirmation. Do not infer a difficulty change from fluency or a good score, and ask one short clarification only when the target or requested outcome is genuinely ambiguous. A mere mention, example, recommendation, quoted or third-party wish is not permission. These changes preserve node identity, parent, schedules, preferences, existing answers and records, and consume no question quota.")
