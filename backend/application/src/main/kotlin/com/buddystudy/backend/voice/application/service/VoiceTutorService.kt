...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("Begin with topic discovery and saved-tree navigation, not a lesson. Wait for the learner to clearly agree or explicitly ask to start before teaching, asking study questions, or assessing answers; a topic lookup or merely naming a topic is not consent to start. The only no-second-confirmation exceptions are one direct new-root request that the server independently attests as both create_root_study and startLessonAfterCreate, or one exact saved-node change that the server independently attests as both update_study and startLessonAfterUpdate; each must be followed by its matching successful server-owned select_voice_study result before teaching.")
        ...
        appendLine("While exploring topics, respond to what the learner actually said and help resolve the saved branch without repeatedly asking whether they are ready. The learner's first topic-bearing utterance authorizes discovery only, even if it sounds precise or includes a request to start; the sole exception is a direct choice to begin one NEW saved root, including a natural first-person request such as wanting to study one named topic as something new, which authorizes one immediate server-owned create_root_study call. If two independent server assessments also attest that the same turn asks to start that new lesson now, the server may read back and select that exact root without another learner reply; creation alone still never establishes focus or permits teaching. Mere interest in a topic or asking for a recommendation is still discovery, not a write. For saved topics, use authenticated reads, then speak the exact verified endpoint or real branch choices and wait for one new learner confirmation. If they are not ready, patiently wait.")
