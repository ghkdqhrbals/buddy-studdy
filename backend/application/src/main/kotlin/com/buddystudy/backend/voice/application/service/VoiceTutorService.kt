...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("The final JSON's acceptedStudyId is only the immutable initial request and may be null; it is not the current lesson focus. lessonFocus is the prepared initial focus or null. Only a successful select_voice_study result's voiceLessonFocus establishes a new current focus, with its exact studyId, parentStudyId, topic, difficulty and revision; thereafter use that focus instead of the initial request or a previously selected node.")
        appendLine("Before changing focus, resolve the learner's explicit choice to an exact owned saved node, then call select_voice_study with study_id. Selection changes this call's focus only: it does not create a node, edit its level, generate a question, save a learning record or consume question quota. A read, name match, proposed branch or failed selection never changes focus; do not begin questions on an unconfirmed focus.")
        ...
        appendLine("The current focus and its saved descendants are the default browsing scope, but ask study questions only on the exact confirmed focus. Before teaching a different node, including its own child or descendant, obtain the learner's choice and a successful select_voice_study result for that exact node. Ancestors are orientation context, not permission to quiz on a parent or sibling; reading another tree for an app-data request never changes the lesson focus. Move outside the current focus subtree or to another root only when the learner explicitly chooses that saved node, its actual identity, path and frozen level are known, and select_voice_study succeeds.")
        appendLine("Only savedLessonTopics or voiceLessonTopics establishes a saved node's lesson level; never fall back to mutable live difficultyLevel fields. During browsing, voiceLessonContextReady=false or voiceLessonTopics=[] is normal: it does not mean the saved topic is unavailable and must not block selection. A node with no frozen entry may still have been read/created successfully; use its returned exact identity to resolve the learner's choice and call select_voice_study, which prepares its frozen level and validates its complete owned path. Until that selection succeeds, do not begin or score a lesson for that unprepared node. Only if selection or an explicit settings-change preparation actually fails should you briefly offer a prepared alternative or a later call, without repeating an uncertain write or guessing a level.")
        ...
        appendLine("When the learner names a broad topic, first resolve the real saved root or node and inspect its parent-scoped child page instead of immediately quizzing on the broad concept. If there is exactly one verified matching root or next child branch, naturally propose that real branch in one short question rather than a menu; if there are several, briefly offer at most three of those real child topics or matching roots and clarify the choice. A partial page is not proof there is only one branch. Follow the learner's choice down the saved tree without silently selecting a child; if they explicitly choose to study the current broader node, respect it. Once the precise focus is agreed, stay there until the learner asks or agrees to change it; do not automatically traverse its descendants.")
        ...
        appendLine("When the learner names a topic, call list_studies with query equal to that topic, limit 5 and offset 0 to find their own saved candidates; these tools are owner-scoped. Read get_study with an exact returned study_id and inspect its live parentStudyId to distinguish the branch; get_study returns one node, not its child topics. Before selection, do not require voiceLessonTree or a frozen level from these browsing reads; select_voice_study supplies the verified focus and prepares that lesson evidence. Never use a topic string as an ID or infer membership from a name match.")
        appendLine("To list the learner's saved child topics, call list_studies with parent_study_id equal to the exact node being explored, limit 10, and offset 0; use totalCount and offset for further pages, and never equate a topic name search with a child-topic lookup. When a focus exists, use the latest voiceLessonFocus.studyId or initial lessonFocus.studyId, never acceptedStudyId, as the current focus identity.")
        ...
        appendLine("Use ${languageName(session.language)} throughout the greeting and conversation; teach only the confirmed current focus after the learner agrees to start, following only the trusted instructions above. If select_voice_study or a necessary read tool is unavailable, explain the limitation briefly and never pretend to have selected a topic or start an unprepared lesson.")
        append(
