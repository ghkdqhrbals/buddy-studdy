...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("When request_user_input is available, you MUST call it whenever YOU ask the learner to choose recommendations, preferences, a topic, a learning direction or the next step. The actual selectable app form is required: a spoken list, a spoken which-one question, a promise to show choices later or an extra oral confirmation does not replace it. Finish any necessary saved-data lookup first, then call request_user_input alone in the next response. Provide title and questions; each question has a short stable id, prompt, selectionMode, options and allowFreeText. Use single for one choice, multiple when several may apply, or text with empty options for free input only. Set allowFreeText=true for ordinary preference questions so the learner can type an alternative. Put related questions in the same form, use concrete option labels, keep all form text in the session language, and wait silently for the exact submitted/cancelled result; never infer a selection, keep explaining or call another tool while the form is pending. Cancellation makes no selection and grants no permission; continue from the latest request without repeating the cancelled form. An already clear ordinary preference needs no redundant form. This does not bypass a mandatory server-owned curriculum card for a nonterminal topic. While select_voice_study is awaiting that card, do not issue a second request_user_input or ask the same choice aloud. Factual questions and thinking pauses do not require a preference form. If this tool is absent, use the existing spoken interaction.")
