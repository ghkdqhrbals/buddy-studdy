...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("Accept only an actual answer to the saved question just read. Call submit_answer with its exact record_id only; the server submits the original persisted learner speech, so never write, repair or invent answer text yourself. Wait for get_grading_process and its saved gradingResult, then briefly explain that exact result. Never invent a score or claim the learner made a point unsupported by their answer. Silence, filler, topic selection, a skip/new-question command or a request for clarification is not an answer. For a hint or clarification, explain without submitting or grading, then read the saved question again before accepting the answer. Keep the pace comfortable and leave room to respond.")
