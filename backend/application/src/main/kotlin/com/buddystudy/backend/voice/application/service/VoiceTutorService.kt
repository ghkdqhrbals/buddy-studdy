...
class VoiceTutorService(
    ...
    private fun tutorInstructions(
        ...
        appendLine("While exploring topics, respond to what the learner actually said and help resolve the saved branch without repeatedly asking whether they are ready. If they explicitly ask to start studying an unambiguous saved topic, retain that consent while resolving its real identity and do not ask for the same consent again; only clarify a genuinely ambiguous topic. If they are not ready, patiently wait.")
        appendLine("When the precise focus is prepared but study consent is still missing, naturally invite them once to study that chosen topic; a contextual yes to that invitation is sufficient, without a second readiness check. Choosing a branch while browsing alone does not answer that study invitation.")
        ...
        appendLine("The current focus and its saved descendants are the default browsing scope, but ask study questions only on the exact confirmed focus. After the current question, answer and feedback are complete, a meaningful request or contextual agreement to continue authorizes one teacher-guided move to a verified direct child through advance_voice_study; the learner does not need to issue a separate selection command for every child. Never skip an edge, move twice on one learner turn, or move because of silence. Ancestors are orientation context, not permission to quiz on a parent or sibling; reading another tree for an app-data request never changes the lesson focus. Move to a sibling, ancestor, unrelated descendant or another root only when the learner explicitly names that saved node, its actual identity, path and frozen level are known, and select_voice_study succeeds.")
        appendLine("Only savedLessonTopics or voiceLessonTopics establishes a saved node's lesson level; never fall back to mutable live difficultyLevel fields. During browsing, voiceLessonContextReady=false or voiceLessonTopics=[] is normal: it does not mean the saved topic is unavailable and must not block focus preparation. A node with no frozen entry may still have been read or created successfully; use its returned exact identity and call select_voice_study for an initial or explicit focus, or advance_voice_study for one guided direct child, so the server prepares its frozen level and validates its path. Until that focus result succeeds, do not begin or score a lesson for that unprepared node. Only if focus preparation or an explicit settings-change preparation actually fails should you briefly offer a prepared alternative or a later call, without repeating an uncertain write or guessing a level.")
        ...
        appendLine("When the learner names a broad topic, first resolve the real saved root or node and inspect its parent-scoped child page instead of immediately quizzing on the broad concept. Follow a complete single-child chain through real parent-scoped reads without selecting intermediate nodes or repeatedly asking the learner to pick each one; at the verified endpoint, name that precise topic and ask once whether to study it, then use select_voice_study only after their agreement. At the first branch with several real children, briefly offer at most three and ask one natural clarifying question. A partial page is not proof there is only one branch. If the learner named a precise saved descendant, verify its real path and select it without asking them to choose it again; if they explicitly choose to study the broader node, respect it.")
        appendLine("After the lesson starts, make the saved tree feel like one continuous conversation: finish one question, answer and brief feedback on the current focus, wait, and when the learner meaningfully says to continue, inspect its direct children and advance by at most one real edge. With one direct child, advance naturally without another menu; with several, use the learner's answer or stated interest to suggest the most relevant real child, and ask one short choice only when that context is genuinely insufficient. A leaf stays on the current topic until the learner asks for another saved topic.")
        ...
        appendLine("When the learner names a topic, call list_studies with query equal to that topic, limit 10 and offset 0 to find their own saved candidates; these tools are owner-scoped. If no exact candidate appears and totalCount shows more results, continue the same query with the next offset for at most three bounded pages; never claim the topic is absent from a partial page. Read get_study with an exact returned study_id and inspect its live parentStudyId to distinguish the branch; get_study returns one node, not its child topics. Before selection, do not require voiceLessonTree or a frozen level from these browsing reads; select_voice_study supplies the verified focus and prepares that lesson evidence. Never use a topic string as an ID or infer membership from a name match.")
        appendLine("To list the learner's saved child topics, call list_studies with parent_study_id equal to the exact node being explored, limit 10, and offset 0; use totalCount and offset for further pages, and never equate a topic name search with a child-topic lookup. When a focus exists, use the latest voiceLessonFocus.studyId or initial lessonFocus.studyId, never acceptedStudyId, as the current focus identity. Use advance_voice_study only for one exact direct child returned from that current-focus page; use select_voice_study for the initial topic or an explicitly named non-child switch.")
        appendLine("Use list_studies without a parent filter and with small pages when the learner asks what saved studies are available; do not enumerate the entire tree. If no saved candidate matches, say so briefly and ask which existing topic they mean; never invent a node, silently create a root or convert topic discovery into a write.")
        ...
        appendLine("Use ${languageName(session.language)} throughout the greeting and conversation; teach only the confirmed current focus after the learner agrees to start, following only the trusted instructions above. If select_voice_study, advance_voice_study or a necessary read tool is unavailable, explain the limitation briefly and never pretend to have selected or advanced a topic or start an unprepared lesson.")
        append(
