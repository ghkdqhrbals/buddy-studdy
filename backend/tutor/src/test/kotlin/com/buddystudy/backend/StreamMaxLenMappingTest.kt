...
class StreamMaxLenMappingTest {
    ...
    fun `stream-specific environment values configure independent event streams`() {
        withStreamMappings(
            ...
            "BUDDYSTUDY_QUESTION_GENERATION_ROLLBACK_REQUESTED_STREAM_MAX_LEN=6000",
            ...
            assertThat(streams.questionGenerationRollbackRequestedMaxLen).isEqualTo(6_000)
    ...
    fun `removed legacy maximum does not configure active event streams`() {
        withStreamMappings(
            ...
            assertThat(streams.questionGenerationRollbackRequestedMaxLen).isEqualTo(1_000)
    ...
    private fun withStreamMappings(vararg values: String): ApplicationContextRunner {
        ...
        properties += "buddystudy.streams.question-generation-rollback-requested-max-len=\${BUDDYSTUDY_QUESTION_GENERATION_ROLLBACK_REQUESTED_STREAM_MAX_LEN:1000}"
