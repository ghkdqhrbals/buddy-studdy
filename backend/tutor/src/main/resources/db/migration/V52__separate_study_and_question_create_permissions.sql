...
select 'question:create', 'Create study questions', true, now(), now()
...
select existing.role_id, question_permission.id, now(), now()
...
where permission.id = requirement.permission_id
  and permission.code = 'study:create'
