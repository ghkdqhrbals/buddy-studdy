...
class SmtpEmailVerificationSender(
    ...
    override fun send(email: String, code: String, ttl: Duration) {
        try {
            ...
            helper.setSubject("BuddyStuddy verification code")
    ...
    private fun fromAddress(): InternetAddress {
        ...
        val personal = parts.dropLastWhile { it != address }.dropLast(1).joinToString(" ").ifBlank { "BuddyStuddy" }
        return InternetAddress(address, personal, Charsets.UTF_8.name())
    }

    private fun body(code: String, ttl: Duration): String =
        """
        BuddyStuddy verification code
