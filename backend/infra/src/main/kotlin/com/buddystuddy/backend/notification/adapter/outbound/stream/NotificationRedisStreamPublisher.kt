package com.buddystuddy.backend.notification.adapter.outbound.stream
...
class NotificationRedisStreamPublisher(
    ...
    init {
        if (properties.streams.enabled) {
            requireNotNull(publisher) { "notificationStreamPublisher bean is required when buddystuddy.streams.enabled=true" }
        }
    }
