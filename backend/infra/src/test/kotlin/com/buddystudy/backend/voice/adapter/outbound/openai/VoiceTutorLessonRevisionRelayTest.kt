...
class VoiceTutorLessonRevisionRelayTest {
    ...
    private class Fixture(initialRevision: Long = 0, finishOpening: Boolean = true) : AutoCloseable {
        ...
        fun done(response: Response, calls: List<String> = emptyList(), spoken: Boolean = false) {
            val output = buildList<Map<String, Any>> {
                ...
                calls.forEach { id -> add(mapOf(
                    "type" to "function_call", "status" to "completed", "call_id" to id,
                    "name" to "update_study", "arguments" to "{}",
                )) }
            }
        ...
        fun assess(decision: VoiceTutorInputDecision = VoiceTutorInputDecision.MEANINGFUL) {
            ...
            controller.completeInputAssessment(assessment.token, Result.success(VoiceTutorInputAssessmentResult(
                assessment.utterances.map { VoiceTutorInputItemAssessment(it.itemId, decision) },
            )))
        }
        fun publish(id: String, persisted: Boolean = true): Long {
            val publication = actions.filterIsInstance<VoiceTutorInputTurnCoordinator.Action.Publish>().last { it.itemId == id }
            assertThat(controller.canPublishInput(id)).isTrue()
            val revision = revision(publication.rawEvent)
