    ...
    async def _run_once_with_lock(self) -> int:
        ...
        for row in self.database.due_schedules():
            ...
            try:
                ...
                pending_count = self.database.pending_record_count(device_id, user_id=user_id, topic=row["topic"])
                ...
                if created_record_id is not None:
                    ...
                    self.database.mark_error(device_id=device_id, error=str(error), user_id=user_id, topic=row["topic"])
