...
- `SPRING_PROFILES_ACTIVE`: runtime profile. Use `dev` for local MySQL plus the OpenAI key from AWS Secrets Manager, `dev-aws` when development must import the entire AWS secret, or `prod`; the default is `dev`.
