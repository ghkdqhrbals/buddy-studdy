...
- Negotiates foreground-only Pro Voice Tutor audio over direct iOS/OpenAI WebRTC with server-owned `realtime-native-v1` sideband control. Learner speech immediately silences the current ordinary tutor response on iOS; the backend cancels that response and clears queued output before replying to the latest stopped, committed learner turn. The native realtime model owns live conversational meaning without waiting for a separate input-assessment call or transcript persistence. iOS bundles Silero for acoustic speech boundaries and keeps microphone capture active during interruption. The bounded legacy PCM fallback retains its existing behavior. Voice time is accounted in seconds, and bounded transcript turns plus a separate private learning result are stored. A separately default-off, explicit-consent flow may retain one private mixed call recording under the lifecycle below.
...
- `OPENAI_REALTIME_MODEL`: realtime voice model; defaults to `gpt-realtime-2.1`, or `gpt-realtime-2.1-mini` in the `dev` profile for the voice cost/quality trial. An explicit environment value overrides either default; use `gpt-realtime-2.1` to roll back the development trial.
...
- `VOICE_TUTOR_CONTINUOUS_SPEECH_INTERVENTION_SECONDS`: legacy-compatible name for the internal ongoing-speech transcription checkpoint interval; defaults to `12` seconds and is clamped to 5–30 seconds. The checkpoint timer never schedules a tutor reply during ongoing learner speech. Native interruption occurs at a speech-start edge; its next normal response waits for the stopped input commit and interruption acknowledgements. Only the legacy path additionally requires meaningful-input assessment and persistence before replying.
...
fence pause/resume; the pause action itself does not cancel or truncate output.
Native speech interruption is separate: the backend sends `response.cancel`
before `output_audio_buffer.clear` and emits the additive
`buddystudy.voice.response.interrupted` control event with its exact `responseId`.
Interrupted responses cannot resume, retry, execute pending tools or contribute
completed tutor transcript evidence. Pause still uses
