...
- `backend/`
  ...
  - Resolves monthly question allowance from `user_entitlement_projection` and the current policy-v3 quota period. An initial paid purchase or verified paid tier change starts a zero-used monthly window. Upgrades carry no unused allowance; downgrades immediately apply the lower base and preserve the previous visible remainder as a one-window bonus. Generation reserves one unit by Saga correlation ID before work, commits it only after a usable question is stored, and releases it exactly once on permanent failure. `GET /api/v1/questions/quota` remains compatible while exposing base, bonus, reserved, period, and policy-version fields; `GET /api/v1/billing/status` is the unified entitlement-plus-quota authority.
...
- Remaining capacity is `max(0, base + current-period bonus - committed - reserved)`. A paid upgrade immediately applies the higher tier and resets committed usage to zero. A paid downgrade immediately applies the lower tier, resets committed usage, and carries the previous visible remainder as a one-period bonus; moved active reservations stay subtracted. A unique invoice-scoped quota-ledger event makes confirmation, webhook, reconciliation, and replay converge idempotently. Legacy membership/usage rows remain dual-written only during migration and are not the final read authority.
