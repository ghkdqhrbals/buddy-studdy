...
- `backend/`
  ...
  - Resolves monthly question allowance from the active membership tier, an optional recurring per-user override, and an optional current-period override stored with the exact usage period. `GET /api/v1/questions/quota` returns usage, the effective allowance, remaining count, and the next UTC reset instant.
  - Provides authenticated admin APIs for paginated user search, tier allowance updates, per-user tier/recurring override assignment, and a current-period-only allowance override. Admin user summaries include the exact current quota-period start and reset instants so the operator UI can preview the scope of a temporary change. Payment-plan metadata is never returned by the consumer quota endpoint.
...
- RevenueCat owns StoreKit transaction completion in both App Store and Test Store builds. The stable BuddyStudy `appAccountToken` is its App User ID, with `original_app_user_id` and aliases accepted for identity recovery. Authenticated RevenueCat webhooks are the primary purchase, refund, cancellation, and expiration path; direct Apple JWS remains a backward-compatible recovery input. Both paths deduplicate with the provider transaction ID before entitlement fulfillment or revocation, and Test Store events are development-only.
- Invoice projection status is only `WAITING`, `COMPLETED`, or `FAILED`; granular verification, fulfillment, cancellation, and compensation steps are event/payment/action states. `COMPLETED` means verified payment plus successful membership/quota entitlement application, not payment approval alone.
