...
- RevenueCat runs in StoreKit 2 observer mode. The stable Apple `appAccountToken` is its App User ID, direct Apple JWS remains the primary payment evidence, and authenticated RevenueCat webhooks provide an at-least-once secondary recovery path. Both paths deduplicate with the Apple transaction ID before entitlement fulfillment.
