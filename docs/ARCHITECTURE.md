...
- `backend/`
  ...
  - Resolves monthly question allowance from `user_entitlement_projection` and the current immutable-anchor quota period. Generation reserves one unit by Saga correlation ID before work, commits it only after a usable question is stored, and releases it exactly once on permanent failure. `GET /api/v1/questions/quota` remains compatible while exposing base, bonus, reserved, period, and policy-version fields; `GET /api/v1/billing/status` is the unified entitlement-plus-quota authority.
...
- A user-initiated purchase creates an `INVOICE_CREATED` event and `NORMAL/WAITING` invoice before the StoreKit sheet opens. The client passes that invoice number back with the signed Apple transaction; the backend verifies the JWS and owns every later state transition.
- The signed-transaction request returns 2xx only after the payment is settled, the invoice records `fulfilled_at`, and matching entitlement plus quota projections are readable. An application failure preserves verified financial evidence for idempotent recovery but returns an explicit error; iOS never collapses it into an approval-pending state.
...
- RevenueCat owns StoreKit transaction completion with the App Store `appl_` public SDK key in every iOS build. Apple determines Sandbox versus Production from the transaction environment. The stable BuddyStudy `appAccountToken` is its App User ID, with `original_app_user_id` and aliases accepted for identity recovery. Authenticated RevenueCat webhooks are the primary purchase, refund, cancellation, and expiration path; direct Apple JWS remains a backward-compatible recovery input. Both paths deduplicate with the provider transaction ID before entitlement fulfillment or revocation.
...
- `quota_accounts` uses account creation until the first verified paid `purchasedAt`, then fixes that first-paid timestamp as the lifetime monthly anchor. Renewal, cancellation, expiration, refund, product changes, and resubscription never move it or reset usage.
- `quota_periods` is a lazy counter projection. `quota_reservations` and append-only `quota_ledger` make reserve/commit/release and bonus changes idempotent. There is no reset batch.
...
- Remaining capacity is `max(0, base + current-period bonus - committed - reserved)`. Upgrade changes only the base limit; downgrade, expiration, and refund keep usage and lower only the limit. Legacy membership/usage rows remain dual-written only during migration and are not the final read authority.
