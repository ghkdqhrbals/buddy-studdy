...
- `backend/`
  ...
  - Pending recovery is declared through `@StreamScheduler`; entries older than the configured idle time are claimed and passed through the same typed handler contract. Push publishing, consuming, and idle recovery are owned together by `PushStreamManager`.
