...
- `backend/`
  ...
  - Stores the latest iOS marketing version, build number, and observation time on each device. `POST /api/v1/app-updates/check` compares numeric semantic-version components before build number, returns only an active outdated campaign, and marks a previously prompted user converted when the target or newer build later checks in.
  - Owns one active update campaign per platform and per-user campaign state in `app_update_campaigns` and `app_update_user_states`. Forced and optional presentation modes share the same localized payload; shown, dismissed, and App Store opened timestamps remain distinct so conversion uses prompted users as its denominator.
  - Exposes authenticated Monitoring APIs under `/api/v1/admin/app-updates` for activation, ending, campaign metrics, and paginated user-level conversion inspection. Activating a campaign transactionally ends the previous active campaign.
...
- The Spring Boot Kotlin backend is the production path for server-scheduled APNs delivery. It stores APNs tokens and schedules in MySQL, keeps user OpenAI keys encrypted at rest when provided, creates due questions with OpenAI, stores the question and `question_push_outbox` entry in one transaction, publishes the outbox through the dedicated `notification.question-push.requested.v1` Redis Stream, and sends APNs alerts from an `@StreamListener` consumer. Successful push messages use `StreamOptions.ACK` so bounded stream history remains available for operations; failures remain pending for `@StreamScheduler` auto-claim recovery.
