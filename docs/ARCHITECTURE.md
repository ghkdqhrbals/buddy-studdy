...
-> iOS POSTs an audio-only offer to /api/v1/voice-tutor/sessions/{id}/webrtc
...
-> iOS sends foreground microphone/model media directly over WebRTC and opens /control with subprotocol buddystudy.voice.control.v2
-> backend alone owns session configuration, VAD turn decisions, response creation, transcript persistence, quota/lifecycle control, and provider hangup
...
- The first release is an authenticated, iOS foreground-only voice surface. The backend owns the OpenAI Realtime call, model instructions, API-key HMAC-pseudonymized safety identifier, credential, quota clock, provider failure mapping, and sideband. iOS receives no provider credential: it exchanges SDP through BuddyStudy, carries only RTP media on the resulting audio-only WebRTC peer connection, and sends lifecycle/playout acknowledgements through BuddyStudy's authenticated control socket.
- iOS snapshots and validates its bounded audio-only SDP before installing the local description, then waits at most 15 seconds for the peer's combined ICE/DTLS connected state before opening control. The GA sideband explicitly sends `session.update` and waits within the configured connection timeout for `session.updated` to confirm `server_vad`, `create_response=false`, and `interrupt_response=false`; an HTTP upgrade or `session.created` alone is not policy readiness. Only after that confirmation does the backend send ready and request exactly one opening response through the same response/playout gate as subsequent turns. Trusted session instructions make that opening a greeting and readiness question, and require learner agreement before starting topic instruction. The PCM fallback begins its opening only after the provider acknowledges the session configuration.
...
- Audio capture remains full duplex while tutor audio plays. Server VAD uses `create_response=false` and `interrupt_response=false`, and the backend turn controller exclusively owns `response.create`. The audio-only SDP prevents a modified client from opening a provider data channel and bypassing that policy. Each trusted tutor response is constrained to one short, complete sentence. Learner speech over tutor output remains captured without stopping, resetting, cancelling, truncating, clearing, or dropping the tutor audio. For WebRTC, the controller opens the next response only after the same response has `status=completed`, OpenAI emits `output_audio_buffer.stopped`, iOS observes the last rendered PCM and waits `outputLatency + ioBufferDuration` before sending `buddystudy.voice.playout.drained`, and the learner's stopped turn is committed. A clear, incomplete response, or missing drain is terminal rather than treated as completed speech. The PCM fallback retains its `.dataPlayedBack` acknowledgement and conservative playback floor. A configurable bounded continuous-speech timer may request one short, complete tutor intervention after a long learner monologue without committing or clearing the live learner buffer; the captured turn commits naturally after the learner stops. Response watchdog and explicit call termination may cancel only a broken or intentionally ending response, never merely because the learner began speaking.
