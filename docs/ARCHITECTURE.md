...
- `backend/`
  ...
  - Provides authenticated admin APIs for paginated user search, tier allowance updates, per-user tier/recurring override assignment, and a current-period-only allowance override. Payment-plan metadata is never returned by the consumer quota endpoint.
