...
- The trusted spoken lesson policy uses each supplied node's frozen 1–10 difficulty for one question, learner answer, brief explicit 0–100 feedback, and a related follow-up. It distinguishes tutor assessment from learner questions and never treats greeting/readiness/hint requests as graded answers. It does not change acoustic/semantic input filtering, media playback, interruption rules, or standard-question workflows.
- Summary contract `voice-tutor-summary-v2` adds `explorations_json` (nullable for older rows, V101) and a backward-compatible `result.explorations` array. One existing summary-model call extracts topic/level/depth, `TUTOR_QUESTION` or `LEARNER_QUESTION` exchanges, optional spoken score, supported feedback, and exact transcript turn IDs. Server checks bind evidence to this session, role and sequence; invalid exchanges are dropped without discarding the overall summary, and unsupported numeric scores are cleared instead of regraded. Only matching session snapshots establish saved study identity/difficulty. Structured JSON is bounded to 12 topics, 12 exchanges per topic, 48 exchanges total and 256 KiB; overflow/malformed provider shapes remain explicit generation failures. Strict schemas, plain-text sanitization and native-safe map serialization apply before persistence. Summary extraction is not proof of model assessment accuracy or independent mastery measurement.
...
- In the other direction, the server-owned WebRTC sideband exposes seven existing MCP tools to the voice LLM as Realtime functions: `list_studies`, `get_study`, `create_study_topic`, `list_records`, `get_record`, `get_topic_stats`, and `get_study_growth`. `VoiceTutorMcpToolPort` is implemented by `McpVoiceTutorToolAdapter`, which reuses the existing schemas and `McpTransportContext` handlers; it neither creates a remote MCP server nor forwards the app bearer to OpenAI. SDP configuration and `session.update` advertise the same definitions, and readiness checks the effective function names, descriptions, schemas and automatic tool choice as well as manual VAD. The authenticated principal is captured at control claim and revalidated against the active device/session and call before each operation. Only child creation is writable, with a bounded ancestry check restricting its parent to the selected study's subtree. Parent-scoped `list_studies` uses an owned parent lookup and exact database page/count, not filtering an unrelated global page.
- `VoiceTutorMcpTurnCoordinator` accepts functions only from the matching successful `response.done`, registers call IDs before work, and bounds eight calls per response, six tool rounds per learner turn and 256 IDs per call. A serial MCP worker runs outside provider receive with a 15-second operation deadline, bounded JSON arguments/results, no automatic replay, and an explicit uncertain-write result on timeout. Each `function_call_output` must receive its exact item/call acknowledgement within 15 seconds before `response.create` continues. A proven function-only response requires no nonexistent audio-stop event; a mixed spoken/tool response still waits for the same response's output buffer to stop. New learner speech/assessment/persistence gates continue to apply, with no audio cancellation, clearing or truncation. Opening and intervention responses cannot execute tools. Only a verified successful created child ID becomes the server-owned `buddystudy.voice.study.changed` event; iOS fetches that exact node and preserves pending/current questions and drafts.
