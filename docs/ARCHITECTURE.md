...
- `MobileRootView`: iOS tabs, onboarding, profile category hub, settings, notification inbox, and study-tree interaction.
  ...
  - Forced updates replace the app with a non-dismissible full-screen gate. Optional updates use a dismissible modal and persist dismissal by campaign through `SettingsStore`. Maintenance is a full-screen gate and retains the hidden developer bypass. Malformed, expired, unsupported, or unavailable Remote Config fails open to the existing backend update and monitoring-status APIs so older control paths remain compatible during rollout.
