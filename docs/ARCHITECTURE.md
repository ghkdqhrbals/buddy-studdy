...
- iOS snapshots and validates its bounded audio-only SDP before installing the local description, then waits at most 15 seconds for the peer's combined ICE/DTLS connected state before opening control. The backend requests exactly one opening response after relay readiness, through the same response/playout gate as subsequent turns. Trusted session instructions make that opening a greeting and readiness question, and require learner agreement before starting topic instruction. The PCM fallback begins its opening only after the provider acknowledges the session configuration.
