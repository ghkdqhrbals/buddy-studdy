...
- `backend/`
  ...
  - Resolves monthly question allowance from `user_entitlement_projection` and the current policy-v4 quota period. A verified upgrade immediately applies the higher tier and opens a new window with committed usage and bonus reset to zero. A downgrade remains pending without changing the current tier or quota until the first verified lower-tier renewal at or after `currentPlanEndsAt`; that renewal opens a new lower-tier window with committed usage and bonus reset to zero. Active in-flight reservations survive either transition, and unused allowance is never carried. Generation reserves one unit by Saga correlation ID before work, commits it only after a usable question is stored, and releases it exactly once on permanent failure. `GET /api/v1/questions/quota` remains compatible while exposing base, bonus, reserved, period, and policy-version fields; `GET /api/v1/billing/status` is the unified entitlement-plus-quota authority.
...
- `quota_accounts` uses account creation until a verified paid purchase. An initial paid purchase or verified paid tier change starts a new monthly window at `purchasedAt`; a same-tier renewal remains idempotent and does not create a duplicate reset.
...
- Remaining capacity is `max(0, base + current-period bonus - committed - reserved)`. A paid upgrade immediately applies the higher tier and resets committed usage and bonus to zero. A requested downgrade keeps the current higher tier and quota unchanged until its renewal boundary; the effective lower-tier renewal then resets committed usage and bonus to zero. Active reservations move to the newly effective window and stay subtracted. A unique effective-invoice-scoped quota-ledger event makes confirmation, webhook, reconciliation, and replay converge idempotently, while a deferred downgrade emits no reset event. Legacy membership/usage rows remain dual-written only during migration and are not the final read authority.
