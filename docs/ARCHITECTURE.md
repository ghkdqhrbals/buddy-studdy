...
- `MobileRootView`: iOS tabs, onboarding, profile category hub, settings, notification inbox, and study-tree interaction.
  ...
  - Forced updates replace the app with a non-dismissible full-screen gate that opens the App Store and remains until the installed version reaches the target. Consumer App Store apps cannot silently install their own updates; automatic installation timing remains owned by iOS/App Store settings. Optional updates use a compact, dismissible center banner and persist dismissal by campaign through `SettingsStore`. Maintenance is a full-screen gate and retains the hidden developer bypass. Malformed, expired, unsupported, or unavailable Remote Config fails open for maintenance and may use only the existing backend app-update API as an update-guidance fallback. There is no monitoring-owned status endpoint or maintenance polling path.
