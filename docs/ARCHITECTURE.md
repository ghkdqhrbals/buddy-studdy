...
- Audio capture remains full duplex while tutor audio plays. Server VAD uses `create_response=false` and `interrupt_response=false`, and a backend turn controller exclusively owns `response.create`. Each trusted tutor response is constrained to one short, complete sentence. Learner speech over tutor output remains captured without stopping, resetting, cancelling, truncating, or dropping the queued tutor audio. iOS seals a response only after `response.done`, tracks every scheduled PCM buffer through AVAudioEngine's `.dataPlayedBack` completion, and sends a response-scoped local playback acknowledgement only after the last buffer is rendered. The controller waits for both the matching completed provider response and that acknowledgement, plus the learner's stopped and committed turn, before requesting one coalesced normal response; a per-delta playback floor and bounded lost-acknowledgement fallback prevent an early or missing acknowledgement from overlapping responses. Client `input_audio_buffer.clear`, `response.cancel`, `conversation.item.truncate`, and legacy barge-in controls are rejected. A configurable bounded continuous-speech timer can request a separate one-sentence tutor intervention after a long learner monologue without committing or clearing the live learner buffer; the captured turn commits naturally after the learner stops. A separate bounded response watchdog cancels and terminates a provider response that never reaches `response.done`. Both REST `ENDING` and WebSocket end issue a server-owned active-response cancel, then follow the same bounded final-audio commit and two-second transcript/response drain before settlement.
