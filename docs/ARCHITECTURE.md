...
- The first release is an authenticated, iOS foreground-only voice surface. The backend owns the OpenAI Realtime call, model instructions, API-key HMAC-pseudonymized safety identifier, credential, quota clock, provider failure mapping, and sideband. iOS receives no provider credential: it exchanges SDP through BuddyStudy, carries only RTP media on the resulting audio-only WebRTC peer connection, and sends local speech boundaries and lifecycle/playout acknowledgements through BuddyStudy's authenticated control socket.
...
- WebRTC can deliver nonempty PCM buffers containing only digital zeros after speech has ended. Such buffers do not advance the device playout deadline; every nonzero sample, including a quiet one-unit sample, still advances it. No RMS/loudness threshold or fixed-delay substitute bypasses the same-response completion/buffer-stop/render/latency gate. Nonzero comfort noise remains conservatively counted. The speaking indicator requires locally rendered audio for an active tutor response, not merely `output_audio_buffer.started`. Bounded iOS diagnostics use the existing app-log store for transport/device states, frame counts, route port types, a zero-volume flag, safe error kind/code, and first-stop source; they never retain PCM, transcripts, device names/UIDs, or raw errors.
...
- Each trusted tutor response is one short, complete sentence. Learner overlap remains captured without stopping, resetting, cancelling, truncating, clearing, or dropping tutor audio. The next WebRTC response requires the same response's `status=completed`, `output_audio_buffer.stopped`, the iPhone's `buddystudy.voice.playout.drained` after its last rendered PCM plus `outputLatency + ioBufferDuration`, and acknowledgement of the stopped learner turn's commit. A genuine clear, incomplete response, or missing drain remains terminal rather than being relabelled successful. The PCM fallback retains provider VAD, its `.dataPlayedBack` acknowledgement, and conservative playback floor. The existing bounded long-monologue timer can request one brief tutor intervention without prematurely committing or clearing the learner's ongoing buffer; the server commits when that utterance stops. Response watchdog and explicit call termination may cancel only a broken or intentionally ending response, never merely because the learner began speaking.
