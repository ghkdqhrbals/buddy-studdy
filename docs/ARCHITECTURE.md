...
- Audio capture remains full duplex while tutor audio plays. The always-installed capture-post-processing tap detects local utterances without storing audio or requiring optional-recording consent. Its bounded RMS/noise-floor detector normalizes the native WebRTC FloatS16 samples explicitly, requires 80 ms onset and 700 ms trailing silence, and has no dependency on tutor speaking state. Only confirmed media readiness, explicit user mute, or teardown gates it. Each utterance emits `buddystudy.voice.input.speech.started` and `.stopped` with the same positive integral `sequence`; the next utterance has a greater sequence. A serial bounded stream preserves order and fails explicitly on overflow. Attempt, account, and socket-generation fences reject stale callbacks. Muting finishes an active utterance once; teardown cancels the old stream.
...
- Each trusted tutor response is one short, complete sentence. Learner overlap remains captured without stopping, resetting, cancelling, truncating, clearing, or dropping tutor audio. The next WebRTC response requires the same response's `status=completed`, `output_audio_buffer.stopped`, and acknowledgement of the stopped learner turn's commit. The provider has finished emitting the previous response before starting the next; the existing receiver/track plays their RTP samples in timestamp order without a response-boundary reset. The old `buddystudy.voice.playout.drained` event is accepted only for client compatibility and cannot open or block a turn. A genuine clear, incomplete response, or missing provider buffer-stop remains terminal rather than being relabelled successful. The PCM fallback retains provider VAD, its `.dataPlayedBack` acknowledgement, and conservative playback floor. The existing bounded long-monologue timer can request one brief tutor intervention without prematurely committing or clearing the learner's ongoing buffer; the server commits when that utterance stops. Response watchdog and explicit call termination may cancel only a broken or intentionally ending response, never merely because the learner began speaking.
