...
17. Membership allowances are TIER1 30, TIER2 300, and TIER3 1,000 questions per monthly window. TIER2 and TIER3 each offer monthly and annual StoreKit subscriptions; both durations grant the same tier allowance. The membership screen exposes one billing-period selector, one tier selection, and one primary subscribe/change action. The active plan is read from RevenueCat customer state. Its visible cancellation action opens RevenueCat Customer Center, including for RevenueCat Test Store subscriptions, with Apple's native subscription management as a fallback. Selecting a lower tier uses StoreKit's same-subscription-group downgrade flow. A paid purchase creates an event-sourced `NORMAL/WAITING` invoice before RevenueCat presents the purchase; only authenticated provider payment evidence followed by successful membership and quota fulfillment may mark it `COMPLETED`. An unpaid `NORMAL/WAITING` checkout is automatically cancelled after ten minutes and projected as `FAILED`, while verified payments and refund invoices are excluded from checkout expiry. A cancelled checkout remains in billing history with a user action that opens RevenueCat Customer Center for purchase restoration or an Apple refund request. RevenueCat owns transaction completion and is the primary authenticated server-delivered purchase, refund, cancellation, and expiration source, deduplicated by provider transaction ID. Direct Apple JWS is retained for backward-compatible recovery. Refunds use separate linked `REFUND` invoices. Invoice status is limited to `WAITING`, `COMPLETED`, and `FAILED`.
...
5. Developer options and debugging popups stay hidden by default. The 4-4-4-4 promotion-code field is available before sign-in; the app compares the embedded developer code's SHA-256 hash locally and persists successful access for the current installation across sign-in and sign-out. TestFlight scopes that unlock to the current version/build, starts every newly installed build with debugging disabled, and requires the developer code again after a build update. On the maintenance screen, tapping the maintenance copy five times within two seconds opens the hidden developer-code sheet; successful entry dismisses the current maintenance gate and opens the app for developer QA without changing the published Firebase policy. TestFlight retains the hidden last-tab long-press debug popup, but it responds only after the local developer code has been accepted for the current build.
