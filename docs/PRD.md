...
17. Membership allowances are TIER1 30, TIER2 300, and TIER3 1,000 questions per monthly window. TIER2 and TIER3 each offer one monthly StoreKit subscription. The membership screen presents the current allowance and both paid tiers as a compact single list with no billing-period selector, plus one primary subscribe/change action only when another tier is selected. The active plan and quota are read only from `GET /api/v1/billing/status`; RevenueCat SDK state is limited to products, purchase, restore, and Customer Center. A paid purchase creates a `NORMAL/WAITING` invoice before RevenueCat presents the purchase, and authenticated payment evidence marks that financial record `COMPLETED` independently from subscription/entitlement projection. Unpaid checkouts expire as `FAILED`; completed charges never regress because projection fails and never trigger an automatic refund. RevenueCat lifecycle events are deduplicated, projected asynchronously, and periodically reconciled. Refunds use separate linked `REFUND` invoices. Monthly quota uses an immutable account-created/first-paid anchor, preserves usage across every subscription transition, and reserves/commits/releases each generated system question exactly once by Saga correlation ID. When iOS submits a matching StoreKit JWS directly, the transaction API returns success only after payment settlement, invoice fulfillment, entitlement, and quota projections are all visible. A partial application returns an explicit error while preserving verified payment evidence for retry; iOS must not convert this failure into an approval-pending message.
