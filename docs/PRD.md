...
10. An operator can queue an `ADMIN_MESSAGE` notification for a selected registered user or for the user/device captured with a feedback submission. The operator supplies the title, Markdown message, and a validated `buddystudy://` destination. Supported presets include a Home message popup, Home, My Studies, Records, Statistics, Settings, and Public Questions; arbitrary web URLs are rejected.
