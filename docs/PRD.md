...
8. On launch and foreground entry, iOS checks the active update campaign with both marketing version and build number. Users already on the target or a newer build see no prompt. A forced campaign blocks interaction behind a non-dismissible prompt, while an optional campaign provides a server-recorded Later action and is not repeatedly shown after dismissal. Opening the App Store and returning on the target version are recorded separately so operators can distinguish intent from completed conversion.
...
12. Monitoring `Manage > App Updates` activates or ends one iOS campaign at a time, localizes update copy in Korean, English, and Japanese, and reports checked, prompted, App-Store-opened, and converted users. `Users & Quotas` also shows the latest reported app version/build and observation time for each user.
