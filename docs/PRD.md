...
6. The backend returns one ordered `items` list whose discriminated item type is either `PUBLIC_QUESTION` or `ADVERTISEMENT`; iOS renders that order without selecting, ranking, or repositioning campaigns. Every advertisement contains its localized disclosure, title, optional body, server-generated `selectionId`, and allowlisted `buddystudy://` deep link. The server requires enough surrounding questions, keeps ads away from the first two rows and the end of the page, and ranks eligible campaigns from business priority, authenticated/anonymous relevance, Bayesian-smoothed deep-link view rate, freshness, and exploration. Daily selection caps, minimum selection intervals, and post-view cooldowns prevent fatigue; 85% of eligible selections exploit the top result and 15% explore the remaining top three. The durable selection history is the ranking source of truth. Opening the deep link publishes one idempotent `NATIVE_AD_VIEWED` event through the transactional Outbox and `community.native-ad.view.v1`; iOS does not maintain ad-ranking history.
