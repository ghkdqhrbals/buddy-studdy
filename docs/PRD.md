...
5. Developer options and debugging popups stay hidden by default. The 4-4-4-4 promotion-code field is available before sign-in; the app compares the embedded developer code's SHA-256 hash locally and persists successful access for the current installation across sign-in and sign-out. On the maintenance screen, tapping the maintenance copy five times within two seconds opens the hidden developer-code sheet; successful entry dismisses the current maintenance gate and opens the app for developer QA. While debugging mode or that maintenance bypass is active, the app does not request or poll the production monitoring status endpoint. TestFlight retains the hidden last-tab long-press debug popup, but it responds only after the local developer code has been accepted.
