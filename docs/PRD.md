...
17. Membership allowances are TIER1 30, TIER2 300, and TIER3 1,000 questions per monthly window. TIER2 and TIER3 each offer monthly and annual StoreKit subscriptions; both durations grant the same tier allowance. The membership screen exposes one billing-period selector, one tier selection, and one primary subscribe/change action. The active plan and quota are read only from `GET /api/v1/billing/status`; RevenueCat SDK state is limited to products, purchase, restore, and Customer Center. A paid purchase creates a `NORMAL/WAITING` invoice before RevenueCat presents the purchase, and authenticated payment evidence marks that financial record `COMPLETED` independently from subscription/entitlement projection. Unpaid checkouts expire as `FAILED`; completed charges never regress because projection fails and never trigger an automatic refund. RevenueCat lifecycle events are deduplicated, projected asynchronously, and periodically reconciled. Refunds use separate linked `REFUND` invoices. Monthly quota uses an immutable account-created/first-paid anchor, preserves usage across every subscription transition, and reserves/commits/releases each generated system question exactly once by Saga correlation ID.
