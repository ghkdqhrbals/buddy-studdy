...
BuddyStudy is a quiet AI tutor for people who use AI heavily but still want to keep their own knowledge sharp. The product asks short questions on a schedule, lets the user answer when convenient, grades the answer with OpenAI, and turns the accumulated record into topic-level learning statistics. Pro members can also hold a live two-way voice lesson with an AI tutor and keep a separate private learning result for each completed lesson.
...
1. An authenticated member with an effective TIER2 or TIER3 entitlement can start a live, two-way spoken lesson for a selected study topic. TIER1 receives a localized Pro-upgrade explanation and no realtime credential or stream allocation.
2. The first release is foreground-only. iOS requests microphone permission immediately before the first lesson, uses the BuddyStudy voice-session API and authenticated WebSocket while the lesson screen is active, and ends the session when the app backgrounds, the device locks, the account changes, or the screen is dismissed. Audio background mode and unattended continuation are out of scope.
3. `GET /api/v1/voice-tutor/status` is the client authority for availability, remaining seconds, reset time, and the operator-configured per-session ceiling. `POST /api/v1/voice-tutor/sessions` derives and atomically reserves the next-session maximum as no more than the smaller of the remaining monthly allowance and that ceiling before opening `/api/v1/voice-tutor/sessions/{id}/stream` with subprotocol `buddystudy.voice.v1`.
4. Voice allowance is counted in integer seconds. The server-owned plan default is 18,000 seconds per monthly window for both TIER2 and TIER3 and zero for TIER1. Operators can change each tier's `monthlyVoiceSecondsLimit` without an app release. `VOICE_TUTOR_MAX_SESSION_SECONDS` independently changes the operational cap for one call, defaults to 3,600 seconds, and is hard-clamped to the provider's 60-minute Realtime session limit. The default 300 monthly minutes can therefore be used across up to five full calls. The app displays rounded minutes but never calculates entitlement or authoritative usage locally.
5. The server timestamps the accepted session, meters decoded PCM admitted by the relay, reserves capacity transactionally, and settles the greater of rounded-up connected time and accepted-audio duration exactly once. It releases unused reservation after normal end or failure and expires abandoned sessions. Concurrent starts cannot reserve the same remaining seconds twice, and client-provided duration never determines charged usage.
...
7. Ending a session explicitly through `POST /api/v1/voice-tutor/sessions/{id}/end`, or implicitly through a terminal stream close, produces one private Tutor Learning Result separate from question-and-answer records. The result contains the study/topic context, session time, bounded transcript-derived summary, strengths, misconceptions or gaps, and recommended next steps. It remains private, is paginated newest-first, and does not enter question-score or topic-growth calculations unless a later product policy defines an explicit assessment mapping.
8. Original microphone and model audio is relayed only for the live exchange and is never persisted by BuddyStudy. Bounded transcript turns and the derived result may be stored for the user's private lesson history, but voice endpoint bodies, WebSocket frames, transcript text, summaries, SDP or provider credentials are excluded from API/external-provider logs, analytics, Sentry attachments, and MCP transport logs.
9. The MCP server exposes authenticated, owner-scoped, read-only `list_voice_tutor_sessions`, `get_voice_tutor_session`, and `get_voice_tutor_quota` tools plus the `buddystudy://voice-tutor/sessions/recent` and `buddystudy://voice-tutor/quota` resources. MCP never starts, streams, reconnects, ends, or extends a voice session.
10. Microphone denial, an interrupted audio route, network loss, quota exhaustion, and provider failure all produce explicit recoverable or terminal states. A reserved session has one relay claim: a disconnect is finalized and a retry creates a new session from the remaining monthly allowance, rather than opening a second provider socket against the old reservation. A final result and quota settlement remain idempotent when end requests race or repeat.
11. A learner who starts speaking over the tutor does not stop or truncate the tutor's current audio. Every ordinary tutor response is one short, complete sentence; the app plays that sentence through while continuing to capture the learner, and the backend answers the coalesced committed learner turn only after the final audio buffer has actually played on the device and the learner has stopped. Client-originated input clearing, response cancellation, and audio truncation are rejected. The separate configurable long-monologue rule still permits the tutor to take the floor with one brief sentence without manually committing or clearing the learner's live audio buffer; that turn commits naturally after the learner stops.
...
8. Voice Tutor learning results are a separate private record type with their own bounded transcript, summary, strengths, gaps, next steps, duration, and session timestamps. They are paginated rather than loaded all at once and are never eligible for public-question publication.
...
5. The app does not call OpenAI directly. API-key validation, question generation, grading, Voice Tutor realtime relay, and lesson summarization go through `https://api.ghkdqhrbals.org`.
...
- Persisting original Voice Tutor microphone or model audio.
