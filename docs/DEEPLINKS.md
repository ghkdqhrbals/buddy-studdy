...
이 문서는 iOS 앱이 해석하는 `buddystudy://` 화면 딥링크와 HTTPS 추천·공개 질문
Universal Link의 이동 결과를 정리한다. 새 링크를 발급할 때는 아래 **권장 URI**만 사용한다.
...
- 일반 화면 이동은 `buddystudy` 스킴을 사용한다. 외부 추천 공유는
  `https://api.ghkdqhrbals.org/referrals/{code}`, 공개 질문 공유는
  `https://api.ghkdqhrbals.org/questions/{questionId}?tl={language}`를 사용한다.
- `buddystudy` 화면 링크의 호스트와 경로 이름은 대소문자를 구분하지 않지만
  ID 값은 원문을 유지한다. 공개 질문 HTTPS 링크에는 아래의 엄격한 규칙을 적용한다.
...
- 새 계정이 필수 약관에 모두 동의해 `ACTIVE`가 될 때 추천인과 신규 가입자에게
  각각 Plus(`TIER2`) 1개월을 지급한다. 계정당 1회이고 자기 추천은 거부하며,
...
## 공개 질문 공유 Universal Link

```text
https://api.ghkdqhrbals.org/questions/987?tl=ko
```

- ID는 선행 0이 없는 양의 64비트 정수인 canonical 공개 기록 ID(`questions.id`)이며 `tl`은 `ko`, `en`,
  `ja`다. `tl`은 웹 UI와 요청할 질문 번역의 언어이며 앱의 저장된 언어 설정을
  바꾸지 않는다. 미지원 값은 웹에서 한국어로 처리하지만 앱의 HTTPS 파서는 거부한다.
- 앱은 정확한 `/questions/{id}` 경로와 canonical 호스트만 받으며 포트,
  사용자정보, fragment, 추가·중복 쿼리, 인코딩한 경로 별칭은 거부한다.
  `tl` 생략은 허용하며 새 공유 링크는 현재 앱 언어의 `tl`을 포함한다.
- 앱의 공유 버튼은 canonical 운영 백엔드에서 조회한 공개 완료 기록에만 표시한다.
  일반 질문(`QUESTION`)은 `graded`, 음성 교환(`VOICE_TUTOR`)은 `completed`여야 한다.
  음성 세션·확장 테이블 ID나 개발 서버 ID로 운영 링크를 만들지 않으며,
  링크와 웹 미리보기는 답변·채점·개인 음성 세션 내용을 보내지 않는다.
- AASA는 기존 `/referrals/*`와 함께 `/questions/*`를 허용한다. 설치된 iOS
  앱은 같은 ID의 공개 질문 상세를 연다. 웹의 앱 열기 버튼과 Smart App Banner는
  기존 `buddystudy://public/questions/{questionId}` 경로를 사용한다.
- 앱 미설치 시 서버는 공개 완료 질문의 주제와 짧은 질문 미리보기, App Store
  설치 버튼, 앱 열기 버튼을 제공한다. 일반 질문의 번역이 아직 없으면 원문을 표시하며,
  음성 교환의 미리보기는 원문 질문과 원문 언어를 유지한다.
  답변·채점·작성자·음성 오디오 정보는 이 전용 조회 DTO에 포함하지 않는다.
- 공개 여부, 삭제 여부, 기록 유형별 완료 상태와 답변 존재, 작성자의 공개 허용 상태를
  요청마다 다시 검사한다. 음성 기록은 소유자가 일치하는 음성 확장 기록과 비어 있지 않은
  질문도 필요하다. 비공개·삭제·미완료·존재하지 않는 질문은 동일한 일반 `404`
  화면을 반환하며 질문별 소셜 메타데이터를 만들지 않는다.
- 랜딩은 조회수와 좋아요를 변경하지 않는다. 크롤러의 링크 미리보기가 홈 피드
  인기를 올리지 않도록 질문 상세 서비스의 조회 이벤트를 호출하지 않는다.
- 응답은 `no-store`로 서버·브라우저 캐시 저장을 금지하고 `noindex,noarchive`를
  제공한다. 외부 메신저가 이미 가져간 미리보기의 사본까지 소급 삭제할 수는 없다.
- 설치 후 App Store가 같은 질문 ID를 자동 전달한다고 가정하지 않는다. 설치 후
  원래 링크를 다시 열도록 웹에 안내한다.

배포 전에는 소스 구현만 존재한다. 백엔드 랜딩과 AASA를 배포하고 이 URL을
해석하는 iOS 빌드를 배포한 뒤, 실제 iPhone에서 앱 설치/미설치, cold/warm launch,
언어 세 가지, 비공개 전환 후 `404`를 확인해야 공유 경로의 운영 완료로 본다.
현재 배포된 `1.1.0 (119)`가 새 HTTPS 질문 경로를 지원한다고 가정하지 않는다.
...
| 공개 질문 Universal Link, 지원 앱 설치됨 | 같은 ID의 공개 질문 상세를 연다. |
| 공개 질문 HTTPS URL, 앱 미설치 | 주제·질문 미리보기와 App Store 이동을 제공한다. 설치 후 원래 링크를 다시 열어 같은 질문으로 돌아온다. |
