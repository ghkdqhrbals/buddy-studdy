...
- Deployments must be module-scoped. Backend API, monitoring, health monitor, and infrastructure/routing changes must be represented as separate workflows or separate module-named jobs. Do not combine backend rollout with Monitoring/Grafana/Loki rollout.
- When changing deployment behavior, update `docs/deploy-repo-template/deployment-modules.md` and the matching deploy workflow template. Backend deploy changes belong in `deploy-backend.yml`; Monitoring management UI and API Logs/Grafana/Loki changes belong in `deploy-macbookair-monitoring.yml`.
